...
export default function SharedProgramView({ sections, submissions }) {
  return (
    <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm animate-in zoom-in-95">
      ...
      {sections.map(sec => {
        // Выводим только апрувнутые Председателем и имеющие один из подходящих статусов
        const accepted = submissions.filter(s => 
          s.sectionId === sec.id && 
          s.headApproved && 
          ['accepted_oral', 'accepted_poster', 'needs_revision'].includes(s.status)
        );
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-3 text-[10px] font-black uppercase tracking-widest">
                      ...
                      <span className={`${p.status === 'needs_revision' ? 'text-amber-500' : 'text-indigo-500'}`}>
                        {p.status === 'accepted_oral' ? '• Устный доклад' : 
                         p.status === 'accepted_poster' ? '• Постерный доклад' : 
                         '• Доклад на доработке'}
                      </span>
                    </div>
                  </div>
